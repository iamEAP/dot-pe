...
function filePathToSitePath(filePath: string): string {
...
function isInfraSitePath(sitePath: string): boolean {
  return (
    sitePath.endsWith("/is/") ||
    sitePath.includes("offline-plugin-app-shell-fallback")
  )
}
...
const pages: Page[] = walk(PUBLIC_DIR, []).map((file) => {
...
const pagesBySitePath = new Map<string, Page>(pages.map((p) => [p.sitePath, p]))

function getPage(sitePath: string): Page | undefined {
...
function resolveToFsPath(url: string): string | null {
...
function readPublicText(relPath: string): string | null {
  const fsPath = path.join(PUBLIC_DIR, relPath)
  return fs.existsSync(fsPath) ? fs.readFileSync(fsPath, "utf8") : null
}

function readPublicXml(relPath: string): cheerio.CheerioAPI | null {
